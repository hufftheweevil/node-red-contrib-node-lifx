...
var constants = require('lifx-lan-client').constants
...
// Light capabilities
const LightCapability = new Enum(['TEMPERATURE', 'COLOR', 'INFRARED'])
...
function convertLifxState(lifxInfo) {
  ...
  var state = _.reduce(
    lifxInfo.color,
    (coll, value, key) => {
      if (!hueStateProperties.hasOwnProperty(key)) return coll

      var stateItem = hueStateProperties[key]

      if (stateItem === true) coll[key] = value
      if (_.isString(stateItem)) coll[stateItem] = value
      return coll
    },
...
/**
 * Check if only light power is toggled
 * @param {object} input
 * @return {boolean}
 */
function isStateChange(input) {
  return _.chain(input).keys().difference(['on']).isEmpty().value()
}
...
LightItem.prototype.initialize = function initialize(callback) {
  ...
  let watch = { d073d5276ae6: 'ADAM DESK LIGHT', d073d55810c6: 'JAY TOWER' }
  let LOG = (...stuff) => {
    // if (watch[this.id]) console.log(watch[this.id], ...stuff)
  }
  ...
  async.series(
    [
      ...
      // Get current IR level (if supported)
        ...
        if (!(capability & LightCapability.INFRARED)) return done(null, null)
        LOG('LightItem.initialize : getting max IR')
        self.lifx.getMaxIR((err, data) => {
          LOG('LightItem.initialize : got max IR', err, data)
    ...
    (err, data) => {
      ...
      // Infrared
      if (
        self.info.capability | LightCapability.INFRARED &&
  ...
  async.series(
    [
      ...
      // Get IR info (if we have the capability)
        ...
        // Check capability
        if (!(self.info.capability & LightCapability.INFRARED)) return done(null, null)

        // Check if getMaxIR returns error
        if (!self.info.getMaxIR_Error) return done(null, { brightness: self.info.maxIRLevel })
    ...
    (err, data) => {
      ...
      if (isUpdated) self.state = newState
...
LightItem.prototype.parseColorRGB = function updateColorRGB(input, output) {
  ...
  // rgb channel
  else if (_.isFinite(input.red) || _.isFinite(input.green) || _.isFinite(input.blue)) {
    changed = true
    ...
    // Get channel values
    if (_.isFinite(input.red)) rgb[0] = input.red
    if (_.isFinite(input.green)) rgb[1] = input.green
    if (_.isFinite(input.blue)) rgb[2] = input.blue
...
LightItem.prototype.parseColorTemp = function parseColorTemp(input, output) {
  ...
  if (!(this.info.capability & LightCapability.TEMPERATURE)) return false
  ...
  // Round the values and make sure that they are in the valid range
  output.kelvin = limitValue(
    Math.round(kelvin),
    constants.HSBK_MINIMUM_KELVIN,
    constants.HSBK_MAXIMUM_KELVIN
  )
...
LightItem.prototype.setColor = function setColor(input) {
  ...
  if (this.parseColorRGB(input, newValues)) hasChanges = true

  // Then, parse temperature
  if (this.parseColorTemp(input, newValues)) hasChanges = true

  // Finally parse brightness
  if (this.parseBrightness(input, newValues)) hasChanges = true

  // On/Off
  newValues.on = this.parseState(input.on)
  if (newValues.on !== isOn) hasChanges = true
  ...
  if (!hasChanges && _.isFinite(input.maxIR)) {
    // No color/state change
  }
  // Power state change
  else if (isStateChange(newValues)) {
    if (newValues.on === true) this.lifx.on(duration)
    else this.lifx.off(duration)
  }
  // Color change
...
LightItem.prototype.setReachable = function setReachable(state) {
  ...
  if (this.info.reachable === state) return
  ...
  var retVal = {
    ...
    info: {
      ...
      capability: LightCapability.enums.reduce((coll, enumItem) => {
        if (self.info.capability & enumItem) coll.push(enumItem.key.toLowerCase())
        return coll
      }, [])
    },
